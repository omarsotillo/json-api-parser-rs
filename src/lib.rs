...
use nom::character::complete::{digit0, digit1, multispace0, one_of};
...
fn json_object(input: &str) -> IResult<&str, Node, JSONParseError> {
    let parser = delimited(
        spacey(tag("{")),
        separated_list(spacey(tag(",")), object_member),
        spacey(tag("}")),
    );
...
fn test_integer() {
    ...
    assert_eq!(
        ...
        Err(nom::Err::Failure(JSONParseError::BadInt))
    );
}
...
fn test_string() {
    ...
    assert_eq!(
        json_string(r#""\ud800""#),
        Err(nom::Err::Failure(JSONParseError::BadEscape))
    );
}
...
fn test_values() {
    ...
    assert_eq!(
        parse_json(r#"{ "a": "b" "#),
        Err(JSONParseError::Unparseable)
    );
    assert_eq!(parse_json(" 56 a"), Err(JSONParseError::Unparseable));

    assert_eq!(
        parse_json("9999999999999999999"),
        Err(JSONParseError::BadInt)
    );
